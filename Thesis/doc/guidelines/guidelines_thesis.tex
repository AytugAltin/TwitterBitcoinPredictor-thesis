...
banner containing the master's degree are defined in \Sref{sec:mastercolors}.
...
\item The following master's programmes are known to \emph{oblige} you to
  mention the option or major topic on the front pages: \begingroup
  \makeatletter \let\kulemt@end@master@def\endinput
  \let\kulemt@div@master\@gobble \newcommand*\kulemt@obsolete@master[3][]{}
  ...
  \def\HandleItem#1#2#3#4#5#6#7#8\@nil{%
    ...
    \ifx\@tempa\@empty\else \def\@tempb{-}\ifx\@tempa\@tempb\else \item
    #5\fi\fi}
  \begin{itemize}
    \input{kulemt.cfg}
  \end{itemize}
...
have colours defined by the Faculty of Engineering Science. These are the
official colours, which means that only the Faculty can change them. If a
master's programme wants to have its colours added or changed, it should contact
the Faculty secretary.

All colours are defined as coordinates in the \acro{CMYK} colour space
...
\def\HandleColors#1#2#3#4#5#6\@nil{%
  \if!#4!\else \par\bigskip
    ...
    \centerline{%
      Background colour: \expandafter\printcolor\kulemt@color@bg\hfill
      Text colour: \expandafter\printcolor\kulemt@color@fg}%
  \fi}
