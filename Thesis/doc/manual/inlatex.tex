...
\Cref{cha:kulemt}. The result can be customised and adapted to the master's
programme guidelines with the class options. Additional functionality is
available through numerous LaTeX packages (cf.\ \Sref{sec:packages}). Just make
...
that its source is not available from \acro{CTAN} but from a local web
...
style is declared by the \cs{bibliographystyle} command, which refers to a file
with extension ``\file{.bst}''. The master's programme guidelines or the thesis
...
If you want a \PDF\ file as a result\,\footnote{Every master's thesis must
